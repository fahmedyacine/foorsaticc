  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-4 relative overflow-hidden ${
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        <div className="absolute -top-[20%] -left-[10%] w-[50%] h-[50%] bg-brand/20 rounded-full blur-[120px] mix-blend-screen animate-pulse" />
        <div className="absolute top-[40%] -right-[10%] w-[40%] h-[40%] bg-accent/20 rounded-full blur-[100px] mix-blend-screen animate-pulse delay-1000" />
      </div>
      ...
      <div className={`w-full max-w-md p-8 rounded-3xl shadow-2xl border backdrop-blur-xl relative z-10 transition-all duration-300 ${
        ...
        <div className="text-center mb-8">
          <h2 className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-brand to-accent mb-2">Welcome Back</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? <Loader2 className="animate-spin h-6 w-6" /> : <span className="flex items-center">Sign In <span className="ml-2 group-hover:translate-x-1 transition-transform">→</span></span>}
          </button>
        </form>

        <div className="mt-8 text-center text-sm">
          <span className={`${theme === 'dark' ? 'text-slate-500' : 'text-slate-600'}`}>New here? </span>
          <Link to="/signup" className="text-brand font-semibold hover:text-accent transition-colors">Create an account</Link>
        </div>
      </div>
    </div>
  );
